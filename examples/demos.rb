...
ppr_costs_stats, ppr_conversions_stats = RBPig.connect(:hadoop_config => File.join(File.dirname(__FILE__), 'hadoop_cluster.xml')) do |pig|
  thirty_days_ago = (Date.today-30).strftime("%Y-%m-%d")
  
  pig.grunt(%[
    ppr_keywords = LOAD '/user/hive/warehouse/ppr_keywords' USING HiveTableLoader('hive.hadoop.forward.co.uk', '10000');
    ...
    ppr_conversions = LOAD '/user/hive/warehouse/ppr_conversions' USING HiveTableLoader('hive.hadoop.forward.co.uk', '10000');
